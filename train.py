...
from funcs import check_mkdir, evaluate, AverageMeter, CrossEntropyLoss2d


ckpt_path = 'ckpt'
exp_name = 'cityscapes_fine'
city = 'bochum'
city_v = 'lindau'
...
def main():
    ...
    sliding_crop = additional_transforms.SlidingCrop(args['crop_size'], args['stride_rate'], cityscapes.ignore_label)
    ...
    train_set = cityscapes.CityScapes('fine', 'train', city=city, joint_transform=train_joint_transform, sliding_crop=sliding_crop,
                                      transform=train_input_transform, target_transform=target_transform)
    train_loader = DataLoader(train_set, batch_size=args['train_batch_size'], num_workers=0, shuffle=True)
    val_set = cityscapes.CityScapes('fine', 'val', city=city_v, transform=val_input_transform, sliding_crop=sliding_crop,
    ...
    f = open(os.path.join(ckpt_path, exp_name, str(datetime.datetime.now()).replace(" ", "_")[:-13] + '.txt'), 'w')
    f.write(str(args) + '\n\n')
    f.close()
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            if (i + 1) % train_args['print_freq'] == 0:
                ...
                print("Iter: ", curr_iter, "Time: ", time.time() - TIME)
...
def validate(val_loader, net, criterion, optimizer, epoch, iter_num, train_args, visualize):
    ...
    gts_all = np.zeros((len(val_loader), int(args['longer_size'] / 2), int(args['longer_size'])), dtype=int)
    predictions_all = np.zeros((len(val_loader), int(args['longer_size'] / 2), int(args['longer_size'])), dtype=int)
